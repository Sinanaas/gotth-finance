...
type PatchRecurringHandler struct {
	BM *managers.BasicManager
}

func NewPatchRecurringHandler(bm *managers.BasicManager) *PatchRecurringHandler {
	return &PatchRecurringHandler{BM: bm}
}

func (h *PatchRecurringHandler) ServeHTTP(ctx *gin.Context) {
	...
	if err := h.BM.UpdateRecurring(id, payload); err != nil {
		swalError(ctx, err.Error())
		return
	}
